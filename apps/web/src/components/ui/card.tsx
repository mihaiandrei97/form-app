...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "bg-card text-card-foreground group/card flex flex-col gap-6 overflow-hidden rounded-none border-2 border-foreground py-6 text-sm shadow-[var(--shadow-brutal)] has-[>img:first-child]:pt-0 data-[size=sm]:gap-4 data-[size=sm]:py-4",
        className,
      )}
      {...props}
    />
...
function CardContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("px-6 py-6 group-data-[size=sm]/card:px-4", className)}
      {...props}
    />
  );
}
